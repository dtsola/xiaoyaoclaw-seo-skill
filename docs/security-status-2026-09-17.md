...
## 0.0 v1.0.2 第二轮扫描：20 条（2026-09-17 11:1x 追加）

复扫给出 **skillspector 20 条**（HIGH 6 / MEDIUM 10 / LOW 4）+ **aig 1 条**。逐条拆解后发现：

### A. 「自己招来的」11 条 —— 扫描器把**我写的文档/证据**当成了技能内容

| 命中 | 位置 | 真因 |
|---|---|---|
| SSRF1「云元数据访问」×5 | `SKILL.md:54`、`scripts/seo-audit.js:25`、`docs/security-status-...md:48/50/52` | 文档里写了"**禁止**访问 169.254.169.254"→ 规则命中该字面量，判定为"访问元数据端点" |
| SDI-2 ×1 | `docs/evidence/security-v1.0.1-...json:7` | 证据 JSON 里**描述了原始 SSRF 问题**，被读成"技能存在该问题" |
| RA2「会话持久化」×3 | `docs/evidence/...json:500/561/592` | 证据里含 "cron / state files" 等词 |
| SQP-3 ×2 | `PROGRESS.md:30`、`docs/security-status-...md:1` | 我自己写的中文过程记录未声明语言可选 |

**处置**：新增 **`.clawhubignore`**（CLI 支持，gitignore 语法），把 `docs/` 与 `PROGRESS.md` **排除出发布包**（发布包 22 → **19 个文件**）；同时把 `SKILL.md`、`scripts/seo-audit.js` 注释里的 **IP 字面量全部改成文字描述**（回环/私有/链路本地/云元数据服务地址…）。
> 教训：**仓库侧的过程记录与安全证据不能进技能包** —— 扫描器不分"代码"与"文档"，写在文档里的"禁止 X"会变成"在做 X"。

### B. 真实待修 4 条

| 命中 | 位置 | 修复 |
|---|---|---|
| **aig T09：DNS 重绑定可绕过公网校验** | `scripts/seo-audit.js:96-129,164-174,185-205` | 已在提交 `378b2a1` 修复：改 `node:http/https` + **`connectGuardedLookup()` 建连时校验实际地址** + 端口白名单 80/443 |
| SDI-4：声明"最小权限"但给了 Bash 与 Env | `SKILL.md:51` | frontmatter **删掉 `Env`**；正文把「最小权限」改写成**逐项权限用途对照**，并明确 Bash 只用于跑本技能自带的只读脚本（不执行用户拼出的任意命令、不读凭据、不改系统配置） |
| RA2 ×2（README 安装说明） | `README.md:49`、`README.en.md:30` | 安装说明去掉"往仓库根拷 `AGENTS.md`/`CLAUDE.md`"这一写法，改为**工具技能目录安装（推荐）+ 项目内技能目录（可选）**，并显式写明"不自动往你的项目写文件" |
| SQP-3 ×3（示例文件） | `jsonld-site.template.json:24`、3 个 `llms.txt.*.example:8` | `inLanguage` 改为中性占位符（不再把 `zh-CN`/`en` 写进模板指令）；llms 示例改为**站点属性 + 明确适用范围**（"本站面向中文读者、以中文为主要发布语言"） |

**验证**：`node --check` 通过 · 守卫套件全绿 · 三站实测输出与之前一致 · 发布包预览确认已无内网/元数据 IP 字面量
