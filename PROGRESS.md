...
## 11:1x 第二轮扫描 20 条 → 拆解 + 修复（提交 `378b2a1` 已含 aig 修复，本轮新增文档/包内容修复）

**新扫描**：skillspector **20 条**（HIGH 6 / MEDIUM 10 / LOW 4）+ aig **1 条** —— 但**约一半是"自己招来的"**：
- **SSRF1 ×5**：我在 `SKILL.md` / 脚本注释 / 安全文档里写了「**禁止**访问 169.254.169.254」→ 扫描器命中该 IP 字面量，判定为"访问云元数据端点"
- **SDI-2 ×1**：`docs/evidence/*.json` 里**描述了原始 SSRF 问题** → 被读成"技能存在该问题"
- **RA2 ×3**：证据 JSON 里出现 cron / state files 等词
- **SQP-3 ×2**：`PROGRESS.md`、安全文档是纯中文过程记录

**处置（关键教训）**：仓库侧的过程记录与扫描证据**不能进技能包** —— 扫描器不分"代码"与"文档"，写在文档里的「禁止 X」会变成「在做 X」
- 新增 **`.clawhubignore`**（gitignore 语法，CLI 原生支持）→ 排除 `docs/` 与 `PROGRESS.md` → 发布包 22 → **19 个文件**
- `SKILL.md` 与脚本注释里的 **IP 字面量全改文字描述**（回环/私有/链路本地/云元数据服务地址）

**真实待修 4 条**
- **aig「DNS 重绑定」**（scripts/seo-audit.js:96-205）→ 已由 `378b2a1` 的连接时校验 + 端口白名单修复（本轮才被扫到，说明上一轮提交的修复还没进扫描）
- **SDI-4**：声明"最小权限"却给了 Bash + Env → frontmatter **删 Env**；正文改为**逐项权限用途对照** + Bash 用途限死说明
- **RA2 ×2**：README 中英安装说明去掉"往仓库根拷 AGENTS.md/CLAUDE.md"写法 → 改为工具技能目录安装（推荐）+ 项目内技能目录（可选）+ 明确"不自动写你的项目"
- **SQP-3 ×3**：`inLanguage` 改中性占位符；3 个 llms 示例改**站点属性 + 明确适用范围**

**验证**：`node --check` ✅ · 守卫套件全绿 ✅ · 三站实测一致 ✅ · 发布包预览确认无内网 IP 字面量 ✅
**待批**：发 **v1.0.3** 复扫
- **11:14 指挥官批「发」→ 已提交 v1.0.3**（回执 pending security scans）；复查任务重挂为 cron `a9688e39`（11:48）
